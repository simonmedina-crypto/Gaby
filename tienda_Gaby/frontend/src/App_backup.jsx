...
const App = () => {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      console.log('Response productos status:', resP.status);
      
      console.log('Haciendo fetch a:', `${API_URL}/historial-ventas`);
      const resV = await fetch(`${API_URL}/historial-ventas`);
      console.log('Response ventas status:', resV.status);
      
      const dataP = await resP.json();
      const dataV = await resV.json();
      console.log('Datos productos recibidos:', dataP);
      console.log('Datos ventas recibidos:', dataV);
      setProductos(Array.isArray(dataP) ? dataP : []);
      setVentas(Array.isArray(dataV) ? dataV : []);
    } catch (e) { 
      console.error("Error conexión Backend", e); 
  ...
  const manejarPistola = (e) => {
    ...
    if (ok) {
      ...
      try {
        // Usamos un sonido simple para evitar problemas con el data URL muy largo
            ...
            <form onSubmit={async (e) => {
              ...
              <div className="space-y-1">
                ...
                <div className="relative flex items-center">
                  ...
                  <input value={formData.barcode} onChange={e => setFormData({ ...formData, barcode: e.target.value })} className="w-full p-3 pl-10 bg-orange-50/30 rounded-xl font-bold border-2 border-dashed border-orange-100 focus:ring-2 ring-orange-500/10 text-sm" placeholder="Escanea aqui..." />
                </div>
              </div>
              <div className="space-y-1">
